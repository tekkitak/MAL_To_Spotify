...
spotify = Blueprint(
    "spotify", __name__, template_folder="templates/api/spotify", url_prefix="/spotify"
)
...
def create_spotify_playlist(name: str = "MyAnimeList openings"):
    ...
    response = cast(
        rq.Response, exec_request(url, headers=headers, data=data, method="POST")
    )
...
def playlist_add_songs(uris: str, playlist_id: str):
    ...
    data = {"uris": uris.split(","), "position": 0}

    response = cast(
        rq.Response,
        exec_request(url, headers=headers, data=json.dumps(data), method="POST"),
    )
...
def get_spotify_song(
    name: str | None = None, artist: str | None = None
) -> None | dict[str, str]:
    url = "https://api.spotify.com/v1/search"
    querystring = {"q": f"track:{name} artist:{artist}", "type": "track", "limit": "1"}
    ...
    response = cast(
        rq.Response,
        exec_request(url, headers=headers, params=querystring, method="GET"),
    )  # FIXME: REMOVE THIS FUCKING BULLSHIT
